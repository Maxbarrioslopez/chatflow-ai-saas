  ...
  try {
    ...
    if (result.errors && result.errors.length > 0) {
      ...
      if (firstError.type === 'FieldMismatch' && result.data.length > 0) {
        // Partial parse, continue with available data
      } else {
        return {
          text: '',
          metadata: {
            ...
            warning: `CSV parse error: ${firstError.message}`,
          },
        };
      }
    }
    ...
    const lines: string[] = [`CSV Data: ${filename || 'unknown'}\n`];
    lines.push(`Columns: ${headers.join(', ')}`);
    lines.push(`Total Rows: ${totalRows}${totalRows > MAX_CSV_ROWS ? ` (showing first ${MAX_CSV_ROWS})` : ''}\n`);
    ...
    return {
      ...
      metadata: {
        parser: 'papaparse',
        originalName: filename,
        warning: `CSV parsing failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      },
    };
  }
}
